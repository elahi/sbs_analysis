...
get_random_sizes <- function(size_bin_vector, count_vector, size_interval, 
  ...
  for(i in 1:length(size_bin_vector)){
    ...
    # Draw a random value according to a uniform distribution
    # min = center of size bin - half of size bin
    # max = center of size bin + half of size bin
    if(distribution == "uniform"){
      size_vector_i <- runif(n = count_i, min = size_bin_i - size_interval, max = size_bin_i + size_interval)
    }
