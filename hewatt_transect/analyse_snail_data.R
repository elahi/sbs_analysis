...
#          spp = ifelse(spp == "Tegula brunnea", "Tegula funebralis", spp), 
#          spp = as.factor(spp))
# unique(dat$spp)

##### REMOVE SPECIES NOT OBSERVED BY HEWATT #####

## It is not clear whether species included by Sagarin, but not included by Hewatt, 
## are true or false zeros
## The most conservative course of action is to assume that they were present in the 30s
## but for whatever reason were not counted
## These differences in species composition are better analyzed with a presence/absence analysis

dat <- dat %>% mutate(era = ifelse(sampler == "Hewatt", "past", "present"))
...
spp_counts_era_wide <- spp_counts_era %>% 
  spread(era, abund)
spp_counts_era_wide
write.csv(spp_counts_era_wide, "hewatt_transect/hewatt_output/spp_counts_era_wide.csv")

spp_common <- spp_counts_era_wide %>% filter(past > 0) 
spp_common
...
spp_counts_year_wide <- spp_counts_year %>% 
  spread(year, abund)
write.csv(spp_counts_year_wide, "hewatt_transect/hewatt_output/spp_counts_era_year.csv")
...
ggsave("hewatt_transect/hewatt_figs/abund_year_total.pdf", height = 10, width = 15)

dat2 <- dat[dat$spp %in% spp_common$spp, ]
...
ggsave("hewatt_transect/hewatt_figs/abund_year_common_spp.pdf", height = 10, width = 10)

##### REMOVE RARE SPECIES #####

spp_common

## T brunnea was probably lumped in with T funebralis in the past
## For simplicity just remove T brunnea (alternatively could lump with T funebralis)
## To keep species, must have counted at least 10 individuals total

n_keep <- 10
spp_keep <- spp_common %>% 
  filter(past > n_keep & present > n_keep)
spp_keep
...
ggsave("hewatt_transect/hewatt_figs/abund_year_raw_subset.pdf", height = 10, width = 10)
...
ggsave("hewatt_transect/hewatt_figs/lrr_year_raw.pdf", height = 10, width = 10)
...
stat_dat %>% distinct(spp, size1, trophic.level)
...
stat_datJ <- stat_dat %>% 
  mutate(size1 = ifelse(spp == "Crepidula adunca" | 
                          spp == "Fissurella volcano", size1 + 1, size1))
...
ggsave("hewatt_transect/hewatt_figs/lrr_all_size_glmm.pdf", height = 3.5, width = 3.5)
...
ggsave("hewatt_transect/hewatt_figs/lrr_herb_size_glmm_effects.pdf", height = 3.5, width = 3.5)

## plot two panels
plot2 <- plot_grid(p2, p1, labels = c("A", "B"), ncol = 2)
save_plot("hewatt_transect/hewatt_figs/lrr_herb_size_glmm_2panel.pdf", plot = plot2,
...
ggsave("hewatt_transect/hewatt_figs/abund_year_raw_subset.pdf", height = 7, width = 7)
