################################################################################
##' @title Hierarchical intercepts model - normal distribution
##'
##' @author Robin Elahi
...
hier1_model <- function(dat, iter_adapt, iter_update, n_chains){
  ...
  if(n_chains == 1){
    inits = list(
      alpha = rep(runif(1, 0, 5), y.n.sites), 
      beta = runif(1, -10, 10), 
      sigma = 1, 
  ...
  ## JAGS model
  sink("sbs_bayes/models/hier1_normal_JAGS.R")
  cat(" 
      model{
      ...
      # likelihood
      for(i in 1:length(y)){
      mu[i] <- alpha[y.group[i]] + beta*era[i] # note that this is on log scale
      y[i] ~ dnorm(mu[i], tau)
  ...
  jm = jags.model("sbs_bayes/models/hier1_normal_JAGS.R", data = data, inits = inits, 
